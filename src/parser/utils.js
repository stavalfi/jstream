...
export function graphByIndexesToObjects(graph) {
  const extendedNodeGraph = graph
    .map(pos => {
      const flowIdObject = pos.flowId && {flowId: pos.flowId};
...
export function groupGraphByFlows(parsedFlows, head) {
  ...
  while (stack.length > 0) {
    ...
    if (!node.hasOwnProperty('flowId')) {
      ...
      if (parentFromSameFlow && parentFromSameFlow.hasOwnProperty('flowId')) {
        node.flowId = parentFromSameFlow.flowId;
        flowMembersByFlowId.get(node.flowId).push(node);
      } else {
        node.flowId = flowId;
        flowMembersByFlowId.set(node.flowId, [node]);
        flowId++;
      }
